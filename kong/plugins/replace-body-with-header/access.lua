...
local function transform_json_body(conf, body_raw, actions, headers)
  local removed
  local renamed
  local replaced
  local replacedHeader
  local added
  local appended
  ...
  if actions.has_content then
    ...
    if actions.replaceHeader then
      for _, name, value in iter(conf.replaceHeader.body) do
        if json[name] ~= nil then
          json[name] = headers[value]
  ...
  if removed or renamed or replaced or replacedHeader or added or appended then
    return true, cjson.encode(json)
  end
end


local function transform_form_body(conf, body_raw, actions, headers)
  local removed
  local renamed
  local replaced
  local replacedHeader
...
local function transform_multipart_body(conf, body_raw, actions, content_type_value, headers)
  ...
  if actions.has_content then
    ...
    if actions.replaceHeader then
      for _, name, value in iter(conf.replaceHeader.body) do
        if parts:get(name) then
          parts:delete(name)
          parts:set_simple(name, headers[value])
  ...
  if removed or renamed or replaced or replacedHeader or added then
...
local function transform_body(conf, body_raw)
  ...
  local actions = {
    ...
    replaceHeader = 0 < #conf.replace.bodyheader,
    add     = 0 < #conf.add.body,
  ...
  if content_type == JSON then
    transformed, body_raw = transform_json_body(conf, body_raw, actions, headers)
  elseif content_type == FORM then
    transformed, body_raw = transform_form_body(conf, body_raw, actions, headers)
...
function _M.execute(conf)
  ...
  transofrm_headers_and_body(conf, body_raw)
end
